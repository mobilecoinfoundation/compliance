...
fn get_non_passing_entries() -> HashMap<String, Option<HashSet<String>>> {
  HashMap::from(
    [
      (String::from("UA"), Some(HashSet::from([String::from("Crimea")]))),
      (String::from("IR"), None),
      (String::from("SY"), None),
      (String::from("CU"), None),
      (String::from("KP"), None),
...
#[cfg(test)]
mod tests {
  use crate::{ConfigError, Location, validate_country_code};
  use crate::us_ofac::get_non_passing_entries;

  #[test]
  fn test_validate_passing_country() {
    assert_eq!(validate_country_code(&Location {
      ...
      match country.clone().1 {
        Some(regions) => {
          for region in regions {
            ...
            assert_eq!(result, error, "Testing country {} and region {}", country.0, region);
          }
        },
        _ => {}
      }
